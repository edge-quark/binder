...
The Molex 206760 antenna is supposed to have good performance from $1710$--$2700\,\text{MHz}$ with resonance at $1834\,\text{MHz}$, but in our lab setup it just reflected everything back. Our $S_{11}$ stayed around $-20\,\text{dBm}$ (basically the generator baseline) across the cellular band, which means really bad impedance mismatch. Most of the reflection was probably at the connectors rather than the antenna itself.

The main problems were likely the crude SMA connections mentioned in the lab manual, environmental effects from the metal bench and other equipment, and the fact that our test board doesn't have the proper matching networks this antenna is designed for.

Despite the terrible $S_{11}$, we still detected ambient signals - specifically a strong peak at $2403\,\text{MHz}$ that's probably Bluetooth or Wi-Fi. This shows that even poorly matched antennas can work as receivers when the signal is strong enough.

Overall, this lab highlighted the gap between spec sheet conditions and real-world measurements. Antenna performance is really sensitive to connections, environment, and mounting. You need proper test equipment (like a vector network analyzer with calibration) to get results that match the specs.
